...
int main()
{
    unsigned short h ;  //Вмещает от 0 до 2^16, что подходит по размеру и предотвращает использования отрицательных значений (При использовании Unsigned char - происходила ошибка, как минимум у меня)
    unsigned short b1 ; // То же самое, что и для высоты
    unsigned short b2 ; //
    unsigned int s; //Вмещает явно более 255 тыс, short - не достаточно т.к 64 тысячи определенно больше 255 тыс
    ...
    if (h <= 255); {
        ...
        if (b1 <= 1000){
            ...
            if (b2 <= 1000){
                ...
                s = ((b1 + b2) * h) / 2;
                    std::cout << "S=" << s << '\n';
                    std::cout << "h-" << sizeof(h) * 8 << " bit, min = " << 0 << ", max = " << std::numeric_limits<unsigned short>::max() << '\n';
